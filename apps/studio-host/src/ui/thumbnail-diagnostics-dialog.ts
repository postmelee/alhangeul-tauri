...
import { diagnosticStatus, paragraph, renderDiagnosticResults } from './thumbnail-diagnostics-results';
...
export class ThumbnailDiagnosticsDialog {
  ...
  constructor(private readonly controller: ThumbnailDiagnosticsController) {
    ...
    title.textContent = 'Windows 썸네일 진단';
    ...
    this.start.textContent = '동의하고 검사';
    const body = document.createElement('div');
    paragraph(body, '공개 테스트 문서만 사용하며 개인 문서를 열지 않습니다. 검사 중 임시 파일과 Windows 썸네일 캐시가 생성될 수 있습니다. 결과는 자동 전송되지 않으며 시스템 설정·파일 연결을 변경하지 않습니다.');
    body.append(this.status, this.results);
    const actions = document.createElement('div');
    actions.className = 'thumbnail-diagnostics-actions';
    actions.append(this.start, this.cancel, this.copy, this.instructions, this.button('닫기', () => this.hide()));
    this.dialog.append(title, body, actions, this.notice, this.fallback);
  ...
  private render(view: DiagnosticView): void {
    ...
    this.renderKey = key;
    this.status.textContent = diagnosticStatus(view);
    this.start.disabled = view.phase !== 'ready';
    this.cancel.disabled = !['running', 'connecting'].includes(view.phase);
    ...
    renderDiagnosticResults(this.results, view.snapshot);
  }
  ...
  private async copySummary(): Promise<void> {
    ...
    try {
      ...
      if (!this.closed) this.notice.textContent = '개인 경로·문서 내용을 제외한 진단 요약을 복사했습니다.';
    } catch {
      if (this.closed) return;
  ...
  private async openInstructions(): Promise<void> {
    ...
    if (!this.closed && !opened) this.notice.textContent = '안내 페이지를 열지 못했습니다. 위의 로컬 안내를 참고하고 신뢰 가능한 배포본 또는 IT 담당자에게 문의하세요.';
  }
}
