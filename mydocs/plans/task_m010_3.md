# Task #3 수행계획서 — rhwp v0.8.2 Stable release pin
...
Alhangeul의 지속 upstream인 `edwardkim/rhwp`를 최신 Stable release `v0.8.2`와 resolved commit `9b16aa9e23f476e2b335d7c029fc9f24a199d63c`에 고정한다. `third_party/rhwp` gitlink, Tauri native Rust lock, studio host의 bundled WASM을 같은 source release로 원자적으로 맞추고 저장소가 직접 검증할 수 있는 provenance lock을 도입한다.
...
upstream `v0.8.2`는 2026-07-26 공개된 정식 hotfix release이며 tag가 가리키는 commit은 `9b16aa9e23f476e2b335d7c029fc9f24a199d63c`이다. `Cargo.toml`과 `rhwp-studio/package.json`도 모두 `0.8.2`를 선언한다. 현재 `v0.7.13`에서 `v0.8.2`까지 core·studio 변경 규모가 크므로 단순 버전 교체가 아니라 adapter 호환성과 기존 기능 회귀를 별도 검증해야 한다.

`v0.8.2` release note에는 browser extension 인쇄 복구와 TAC 인라인 표 렌더 정정 외에 studio E2E `print-pdf-issue3126`, `issue-2214`의 알려진 실패가 명시되어 있다. 이 task는 해당 upstream known issue를 임의로 수정하거나 성공으로 간주하지 않고, Alhangeul 검증 결과와 분리해 기록한다. 작업 중 더 최신 release가 나오더라도 이 task는 승인된 `v0.8.2` 기준을 임의로 변경하지 않는다.

## 범위

### 포함

- `third_party/rhwp` gitlink를 `v0.8.2` resolved commit으로 고정한다.
- 동일 checkout을 기준으로 `apps/desktop/src-tauri/Cargo.lock`의 `rhwp 0.8.2` native dependency graph를 갱신한다.
...
- `v0.8.2`로 인한 Alhangeul adapter 호환성 문제만 최소 범위로 보정한다.
- release note에 공개된 upstream known issue와 Alhangeul에서 새로 발견된 회귀를 분리해 검증 결과에 기록한다.
- 공식 upstream·개발 문서에 갱신, 검증, 실패 시 rollback 절차를 반영한다.

### 제외

- `third_party/rhwp` 내부 source 직접 수정 또는 Alhangeul 전용 fork 운영
- `v0.8.2` 범위를 넘어선 최신 release 추종
...
- bundled WASM은 기존 binary를 버전 문자열만 바꿔 재사용하지 않는다. `v0.8.2` checkout에서 upstream의 Docker/`wasm-pack --target web --release` 절차로 새로 생성한 package만 반영하고 lock의 artifact metadata와 연결한다.
...
- **Stage 2 — v0.8.2 source·native·WASM 원자적 고정**
  - submodule gitlink, Cargo lock, fresh WASM package를 `v0.8.2` commit 하나에 맞춘다.
  - `rhwp-core.lock`, 정합성 검증 script와 artifact regression test로 source/artifact 관계를 고정한다.
- **Stage 3 — adapter 호환성 및 공식 운영 문서 정리**
  - studio test/build로 `v0.8.2` API 호환성을 확인하고 필요한 wrapper 최소 보정만 수행한다.
  - release note의 upstream known issue와 Alhangeul adapter에서 새로 발생한 실패를 구분한다.
...
- `rhwp-core.lock`, submodule gitlink, Cargo package version과 bundled WASM version·hash가 `v0.8.2`/`9b16aa9e23f476e2b335d7c029fc9f24a199d63c` 기준으로 일치한다.
...
- **upstream API drift**: `v0.7.13`에서 `v0.8.2` 사이 core·studio 관련 diff가 423개 파일 규모이므로 adapter build나 기존 동작이 깨질 가능성이 커졌다. wrapper의 최소 호환 변경만 포함하고 upstream source 수정이나 새 기능 구현은 분리한다.
- **upstream known issue**: `v0.8.2`에는 studio E2E `print-pdf-issue3126`과 `issue-2214` 실패가 알려져 있다. 이를 Alhangeul 회귀와 혼합하거나 조용히 면제하지 않고 release note 근거와 검증 한계에 명시한다.
...
- `v0.8.2`와 commit `9b16aa9e23f476e2b335d7c029fc9f24a199d63c`을 이번 task의 고정 target으로 사용한다.
...
- WASM은 동일 `v0.8.2` checkout에서 fresh release build한 결과만 반영한다.
- 네 단계로 작업하고, adapter 수정은 `v0.8.2` 호환에 필요한 최소 범위로 제한한다.
- `v0.8.2` release note의 알려진 studio E2E 실패는 별도 검증 한계로 기록하고 이번 task에서 upstream source를 수정하지 않는다.
