...
작업지시자의 이번 “진행해줘”는 수행계획 승인과 구현계획 보정 지시로 기록한다.
이 문서의 승인 전에는 Stage 4.8을 시작하지 않는다. 공개·위험 수용·원격 실행 승인은 별개다.
이번 편집은 수행계획의 승인 상태, 이 구현계획, 오늘할일만 변경한다.
...
새 workflow 개발이나 #19/#28 일괄 완료를 첫 공개의 암묵적인 선행 조건으로 만들지 않는다.
