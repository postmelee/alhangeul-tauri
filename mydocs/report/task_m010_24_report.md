...
| `mydocs/working/task_m010_24_stage*.md` | Stage 1~6 실행·검증·제약·handoff 증적 | 단계별 장기 기록 |
| `mydocs/orders/20260813.md`, `mydocs/orders/20260815.md` | Task #24 진행·완료와 병렬 작업 상태 | 일일 작업 보드 |

전체 변경은 원래 `devel` 기준 37 files, 4,135 insertions, 87 deletions이다. 이 중
...
| 단계 증적 | `mydocs/working/` | `mydocs/working/task_m010_24_stage1.md`~`stage6.md` | OK | 실행 결과를 제품 문서와 분리 |
...
| pin·제품 경계 | OK — product boundary 200 files, product/release version `0.1.0`, pin verifier 통과 |
...
| 플랫폼 중립 회귀 | OK — automation 120/120, upstream 35/35, Studio 105/105 |
...
| PR 준비 상태 | OK — final integrated gate와 `git diff --check` 통과, 승인된 파일만 commit 예정 |
...
- Task #34에서 Linux exact-SHA GUI acceptance를 native x64 GitHub Actions로 자동화한다.
- Task #35에서 Windows Tauri WebDriver·WinApp CLI exact-SHA acceptance를 도입한다.
- Task #24 merge 뒤 사용자 다운로드·release 공지와 canonical URL을 위한 GitHub Pages
  전용 Issue를 등록하고 `postmelee/alhangeul-macos`의 공개 방식과 업데이트 안내를 검토한다.
- Pages/canonical URL 경계를 확정한 뒤 Issue #16 updater를 별도 승인 task로 진행한다.
...
- 작업지시자가 2026-08-15 이 스레드에서 #24 Stage 5·6 완료와 PR 생성까지 연속 진행을
  승인했다.
- 최종 통합 검증과 clean status를 확인한 뒤 `publish/task24` 원격 branch를 fast-forward하고
  `devel` 대상 Open PR을 게시한다.
