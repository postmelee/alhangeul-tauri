...
PR #53 리뷰 뒤에는 수동 다운로드를 인자 없는 native canonical URL command로 닫고, Linux AppImage
실효 `W_OK` 판정, runtime과 동일한 exact release basename, 다운로드 전 dirty guard를 보강했다.
사용하지 않는 release overlay 생성기는 제거했으며 최종 exact-SHA 일반 Windows/Linux artifact
build와 Windows MSI·NSIS smoke를 다시 통과했다.

## 변경 파일 목록과 영향 범위

| 경로 | 변경 요약 | 영향 범위 |
|---|---|---|
| `apps/desktop/src-tauri/src/updater/` | updater 상태·명령·service·target 판별, canonical 수동 fallback과 dirty-session 보호 구현 | Windows x64 MSI·NSIS, Linux x64 AppImage native runtime |
...
| `scripts/updater/` | exact basename release/acceptance inventory, signature·asset read-back, manifest와 native 수용 도구 구현 | 릴리스 자동화와 수용 검증 |
...
최신 `origin/devel`을 병합한 최종 비교 기준으로 83개 파일, 11,467줄 추가, 54줄 삭제다.
...
| 최종 통합 Studio 검증 | updater/lifecycle 통합 전 | production build 및 24 files·132 tests 통과 |
...
| 수용 기준 | 결과 |
|---|---|
| Rust가 updater 상태·target·dirty guard·설치를 소유하고 TypeScript는 좁은 UI bridge만 사용 | OK — Stage 1·2 unit test와 최종 Studio build·132 tests 통과 |
| MSI·NSIS·AppImage가 자기 target만 선택하고 불명확한 설치는 수동 fallback | OK — target fixture, Linux effective-ID `W_OK` probe와 세 형식 native positive/cross-format 수용 통과 |
...
| installer와 `.sig`의 version·target·URL·signature·exact basename이 완전한 inventory에서만 manifest로 투영 | OK — Stage 3 release/Pages/workflow negative 계약, Stage 4 원격 read-back과 Stage 6 filename drift test 통과 |
...
| PR 리뷰 보정의 최종 Windows/Linux artifact 회귀 | OK — exact source `50e91247841b47d5dc50773c0a2584720829dbdc`의 run 33734252261에서 세 platform build·desktop test·Clippy와 Windows MSI·NSIS smoke 통과, publish job skipped |
...
`devel` 통합과 Stage 6 보정은 N→N+1 artifact·manifest·download·install 흐름을 변경하지 않아 이미
통과한 장시간 positive·negative run을 반복하지 않았다. 대신 최종 source의 일반 artifact workflow로
Windows/Linux native test·Clippy·bundle과 Windows installer smoke를 다시 검증했다.
...
- Stage 6: [`task_m010_16_stage6.md`](../working/task_m010_16_stage6.md) — canonical 수동 fallback, Linux 실효 권한과 exact filename 계약 보정 및 최종 exact-SHA artifact 검증 완료.
...
- canonical 수동 다운로드 command는 Windows/Linux compile·Clippy와 UI dispatch unit test를
  통과했지만 실제 OS 기본 브라우저 UI 자동화는 이번 비게시 artifact run 범위에 포함하지 않았다.
...
- 최종 보고서와 Stage 6 수용 기준 검증 결과를 승인하면 PR #53의 최종 검토·병합 절차로 진행한다.
