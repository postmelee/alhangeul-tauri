...
- 대상 이슈: #34
- 마일스톤: M010
- 단계 수: 5개 본 Stage + 6개 post-merge/review 하위 Stage
...
| `mydocs/plans/task_m010_34*.md`, `mydocs/working/task_m010_34_stage*.md`, `mydocs/orders/2026081*.md` | 승인 계획, 5개 본 Stage·6개 하위 Stage 증적과 작업 보드 | Hyper-Waterfall 작업 기록 |
...
| 전체 automation 통과 수 | Stage 1 완료 시 162개 | Stage 5.6 보정 후 201개 |
...
| 실제 hosted Linux x64 GUI | MISS — merge SHA `4ab82ed` native build run `32097086884`는 전 matrix·installer smoke를 통과했지만 GUI run `32097898631`은 제품 실행 전 CUPS A4 문자열 assertion에서 실패했다. Stage 5.6 correction merge 후 live close gate에서 다시 확정한다. |
...
- Stage 5.6 correction은 아직 default branch에 없어 actual `lpoptions -l` 출력과 제품 GUI·WebKitGTK·GTK·AT-SPI·CUPS-PDF 성공을 주장할 수 없다.
...
- Stage 5.6 correction PR merge 뒤 Issue #34 live close gate: same-SHA native build, Linux GUI dispatch, evidence hash 재검산과 시각 read-back.
...
- 갱신된 최종 보고서와 Stage 5.6 수용 기준을 승인하면 `devel` 대상 correction PR을 리뷰·merge하고, merge 후 live Linux x64 close gate를 진행한다.
