...
- 대상 이슈: #45
- 마일스톤: M010
- 단계 수: 4개 Stage, 작업지시자 시각 피드백 보정 19회
...
| `scripts/build-pages.mjs`, `scripts/check-pages.mjs`, `scripts/pages/` | 허용 파일만 복사하는 deterministic build와 schema/link/inventory 검사 | Pages 생성·게시 전 gate |
...
| `.github/workflows/pages.yml` | 40자리 `deploy_ref`, exact checkout, immutable action과 build/check/test/deploy 순서 | 보호 브랜치 기반 Pages 배포 |
...
| deterministic Pages inventory | source 3개·root asset 4개·output 7개 | source 11개·root asset 4개·output 15개 |
| 제품/공유 이미지 | 0개 | provenance가 기록된 PNG 5개 |
| release data | 없음 | `unreleased` JSON 1개, installer target 3개, manifest 비게시 |
| Pages focused 자동 검증 | 없음 | design/build/release metadata 40개 통과 |
| 전체 automation | 기존 Pages 계약 없음 | 254개 통과, 실패·skip 0 |
| task diff(최종 보고서 전) | 해당 없음 | 30파일, +3,032/-322줄 |
...
| Pages build·output | OK — source 11개·root asset 4개·output 15개, broken link·asset drift 없음 |
| 공개 전 negative gate | OK — MSI·NSIS·AppImage direct URL과 `updater/stable.json` 없음 |
| automation | OK — 254개 통과, 실패·skip 0 |
| upstream | OK — 35개 통과, 실패·skip 0 |
| Studio | OK — Vitest 23파일·105개와 TypeScript/Vite production build 통과 |
| product boundary | OK — 추적 파일 격리 snapshot 244개 통과 |
| desktop/mobile UI | OK — 1,280×720과 390×844에서 플랫폼 전환, 동일 행, 버튼 전용 클릭·focus와 overflow 부재 확인 |
| exact-SHA workflow 계약 | OK — exact input/checkout, immutable action, non-cancelling Pages deploy test 통과 |
...
직접 `pnpm run check:product-boundary`는 사용자 소유
`.claude/worktrees/pr-review-dc9f5b`의 과거 checkout을 repository 하위에서 함께 스캔해 실패했다.
해당 worktree를 수정·삭제하지 않고, 현재 commit의 추적 파일만 담은 격리 snapshot에서 같은 script를
...
- canonical Pages는 아직 이번 PR source를 게시하지 않았다. 병합 전에는 새 UI가 공개됐다고
  안내하면 안 된다.
...
- PR 병합 뒤 exact `devel` merge SHA를 workflow ref와 `deploy_ref`에 동일하게 사용해 Pages를
  배포하고 root·updates·feedback·asset·link·release JSON을 public read-back한다.
...
- 최종 보고서와 PR의 source·검증 결과를 리뷰하고 `devel` merge 여부를 승인한다.
- merge 뒤 Pages exact-SHA 배포·public read-back은 별도 운영 gate로 진행한다.
