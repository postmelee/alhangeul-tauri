...
PR 리뷰 후에는 install/remove hook을 분리하고 upstream 2.5 fixture, 실제 stale
cache 복구와 dependency command 없는 DEB purge까지 package evidence로 고정했다.
...
| `apps/desktop/src-tauri/linux/alhangeul.thumbnailer`, `update-mime-database*.sh` | canonical registration과 strict install/tolerant remove refresh | Linux package lifecycle |
...
| `mydocs/plans/`, `mydocs/working/`, `mydocs/report/`, `mydocs/orders/` | 승인, 6단계 결과와 최종 추적 문서 | Hyper-Waterfall 작업 기록 |

전체 diff는 최신 `devel` 기준 **52 files, +3,995 / -466 lines**다. 제품 renderer,
...
| 단계·최종 보고 | `mydocs/working/`, `mydocs/report/` | Stage 1~6, 본 보고서 | OK | long-lived 결과와 위험 기록 |
...
| 플랫폼 중립 automation | 이슈 전 계약 | 413 pass, 0 fail |

## 검증 결과

Stage 5 source/workflow candidate는
`a07bd1330363ee767b9e1cc7a80bed6a685cebcf`였다. PR 게시 뒤 최신 `devel`
`28d01b9a4e1a642b3834755cfe3623c6eb543b39`를 통합한 수용 baseline은
`75f9f8c1a87c6e42e514254c82d9169aa3f5bbea`였다. PR 리뷰 보정까지 포함한 최종
source/workflow candidate는 `dbf09404e8b2e4fd07f510ddc60329e71a596643`, rhwp
pin은 `496333b27d21ddb9114ba9ae340bcb895870c9a7`이다. 아래 최종 native와 GUI
workflow는 모두 이 candidate를 checkout하고 exact SHA를 검증했다.
...
| uninstall 제품 항목 제거와 제3자 상태 보존 | OK — x64 DEB/RPM·arm64 DEB의 11/10/11개 lifecycle 관측에서 제품 세 경로 제거, 기본 앱·제3자 sentinel 불변 |
| remove dependency 부재와 실제 stale cache 복구 | OK — DEB purge는 `update-mime-database` 없이 성공, refresh exit 42 뒤 baseline cache를 관측하고 candidate reinstall로 복구 |
| upstream 세대와 archive 소유권 독립성 | OK — shared-mime-info 2.5 원본 fixture와 제품 XML의 byte 독립성, `/usr/share/mime` 제품 XML 단일 allowlist 판정 |
...
- `pnpm run check:product-boundary`: **332 files scanned, passed**.
...
- `pnpm run test:automation`: **413 pass, 0 fail**.
- `git diff --check`: OK.

지원 대상 최종 native 검증은
[Alhangeul Desktop run 33607431684](https://github.com/postmelee/alhangeul-tauri/actions/runs/33607431684)에서
...
[Linux GUI run 33610310800](https://github.com/postmelee/alhangeul-tauri/actions/runs/33610310800)는
전체 성공했다. GUI artifact `alhangeul-linux-gui-33610310800`은 ID
`9838928525`, SHA-256
`1e3dcf66d04d5c249925392234f87845d2330048c7a656f323e62dd816c92f11`이다.
workflow context는 build ref와 acceptance ref가 모두 final candidate이고 native run
ID가 `33607431684`임을 기록한다. Nautilus·Thunar 모두 실제 HWP/HWPX first
...
Stage 4, Stage 5와 최신 `devel` 통합 뒤에도 동일하게 재현됐다. Stage 6의
진단 run `33595872896`과 `33605957600`, 잘못 확장한 SHA 입력으로 즉시 취소한
run `33605922043`은 어떤 수용 근거에도 포함하지 않았다.
...
- [Stage 6](../working/task_m010_50_stage6.md): remove hook, upstream fixture,
  stale cache·DEB purge evidence와 PR 리뷰 보정 exact-SHA 재수용 완료.
...
- 같은 날 PR 리뷰 검토 뒤 반복한 "진행해줘"와 "이어서 진행해줘"를 Stage 6
  보정, exact-SHA native/GUI 재수용, 보고서와 기존 PR 본문 갱신 승인으로 기록한다.
