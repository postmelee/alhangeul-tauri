...
`변경 내역`은 본문에 Stage timeline만 두고, `영향 영역`과 `작업 문서`를 하위 목차로 분리한다. 이렇게 하면 Stage 흐름이 PR 상단에 한눈에 들어오고, 영역 표와 문서 링크는 GitHub 사이드바 ToC에서 따로 노출된다.
...
- 실제 수행한 검증만 적고, 명령 나열로 끝내지 않는다.
- `자동 검증`, `수동/시나리오 검증`, `CI/원격 검증`, `검증 한계` 하위 섹션을 사용한다.
- 자동 검증은 `주제 / 검증 방법 / 결과 / 근거` 표로 적는다.
  - `주제`: 어떤 수용 기준, 리스크, 변경 영역을 검증했는지 적는다.
  - `검증 방법`: 실행 명령 또는 확인 방법을 적는다.
  - `결과`: `OK`, `MISS`, `SKIP` 중 하나를 사용한다.
  - `근거`: 핵심 출력, 통과 개수, 확인한 조건을 적는다.
- 수동/시나리오 검증은 `시나리오 / 확인 절차 / 결과 / 자료` 표로 적고, 어떤 화면·파일·산출물에서 무엇을 확인했는지 적는다.
- CI/원격 검증은 `항목 / 결과 / 근거` 표로 적고, GitHub Check 이름, run 링크 또는 확인 시점을 남긴다.
- 실행하지 않은 검증은 표에 남기지 말고 `검증 한계` 또는 `남은 리스크`에 사유를 적는다.
- 긴 로그는 PR 본문에 붙이지 말고 최종 보고서나 단계 보고서로 넘긴다.
- 해당 없는 하위 섹션은 통째로 삭제한다.
...
- **[Stage 1](https://github.com/postmelee/alhangeul-tauri/blob/{sha}/mydocs/working/task_m050_22_stage1.md)** ([abc1234](https://github.com/postmelee/alhangeul-tauri/commit/{stage1_sha})): {Stage 1 요약}
- **[Stage 2](https://github.com/postmelee/alhangeul-tauri/blob/{sha}/mydocs/working/task_m050_22_stage2.md)** ([def5678](https://github.com/postmelee/alhangeul-tauri/commit/{stage2_sha})): {Stage 2 요약}
...
- 수행 계획서: [task_m050_22.md](https://github.com/postmelee/alhangeul-tauri/blob/{sha}/mydocs/plans/task_m050_22.md)
- 구현 계획서: [task_m050_22_impl.md](https://github.com/postmelee/alhangeul-tauri/blob/{sha}/mydocs/plans/task_m050_22_impl.md)
- 최종 보고서: [task_m050_22_report.md](https://github.com/postmelee/alhangeul-tauri/blob/{sha}/mydocs/report/task_m050_22_report.md)
...
| 주제 | 검증 방법 | 결과 | 근거 |
|------|-----------|------|------|
| 템플릿 구조 | `grep -nE '자동 검증|검증 한계' .github/pull_request_template.md` | OK | 필수 하위 목차 확인 |
| whitespace | `git diff --check` | OK | 경고 없음 |

### 수동/시나리오 검증

| 시나리오 | 확인 절차 | 결과 | 자료 |
|----------|-----------|------|------|
| PR 본문 검증 가독성 | 작성된 PR 본문에서 자동 검증과 검증 한계가 분리되는지 확인 | OK | PR 본문 미리보기 |
