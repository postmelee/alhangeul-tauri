...
1. 중복 이슈 확인
   ```bash
   gh issue list --repo postmelee/alhangeul-tauri --state all \
     --search "{작업 키워드}" \
     --limit 20 \
...
2. 열린 milestone 목록 확인
   ```bash
   gh api repos/postmelee/alhangeul-tauri/milestones \
...
3. 기존 label 목록 확인
   ```bash
   gh api repos/postmelee/alhangeul-tauri/labels --paginate \
...
8. 승인 후 이슈 생성
   ```bash
   gh issue create --repo postmelee/alhangeul-tauri \
     --title "{제목}" \
     --body "{본문}" \
     --milestone "{milestone}" \
     --label "{label}"
...
9. 생성 결과 확인
   ```bash
   gh issue view {N} --repo postmelee/alhangeul-tauri \
     --json number,title,state,milestone,labels,url
   ```
